...
func (c *ConnectionHandler) jsonFromEEBUSJson(json []byte) []byte {
...
func (c *ConnectionHandler) process_eebus_json_hierarchie_level(data interface{}) interface{} {
	switch data.(type) {
	case *ordered.OrderedMap:
		...
		for {
			...
			var new_value = c.process_eebus_json_hierarchie_level(pair.Value)
	...
	case []interface{}:
		...
		for _, value := range data.([]interface{}) {
			var new_value = c.process_eebus_json_hierarchie_level(value)
...
func (c *ConnectionHandler) jsonIntoEEBUSJson(data []byte) (string, error) {
	...
	var result = c.process_eebus_json_hierarchie_level(temp)
