...
func TestVaVHANSuite(t *testing.T) {
	suite.Run(t, new(VaVHANSuite))
}

type VaVHANSuite struct {
	suite.Suite
	...
	heatingAreaEntity spineapi.EntityRemoteInterface

	eventCalled bool
}

func (s *VaVHANSuite) Event(ski string, device spineapi.DeviceRemoteInterface, entity spineapi.EntityRemoteInterface, event api.EventType) {
	s.eventCalled = true
}

func (s *VaVHANSuite) BeforeTest(suiteName, testName string) {
	s.eventCalled = false
	...
	s.sut = NewVHAN(localEntity, s.Event)
	s.sut.AddFeatures()
	s.sut.AddUseCase()

	s.remoteDevice, s.heatingAreaEntity = setupDevices(s.service, s.T())
}
		...
		EntityInformation: []model.NodeManagementDetailedDiscoveryEntityInformationType{
			{
				Description: &model.NetworkManagementEntityDescriptionDataType{
					...
					EntityType: util.Ptr(model.EntityTypeTypeHeatingCircuit),
				},
			},
		},
