...
func TestCaCRCTSuite(t *testing.T) {
	suite.Run(t, new(CaCRCTSuite))
}

type CaCRCTSuite struct {
	suite.Suite
...
func (s *CaCRCTSuite) Event(ski string, device spineapi.DeviceRemoteInterface, entity spineapi.EntityRemoteInterface, event api.EventType) {
	s.eventCalled = true
}

func (s *CaCRCTSuite) BeforeTest(suiteName, testName string) {
	s.eventCalled = false
	...
	s.sut = NewCRCT(localEntity, s.Event)
	_ = s.sut.AddFeatures()
	s.sut.AddUseCase()
