...
func TestCaCRCSFSuite(t *testing.T) {
	suite.Run(t, new(CaCRCSFSuite))
}

type CaCRCSFSuite struct {
	suite.Suite
...
func (s *CaCRCSFSuite) Event(ski string, device spineapi.DeviceRemoteInterface, entity spineapi.EntityRemoteInterface, event api.EventType) {
	s.eventCalled = true
}

func (s *CaCRCSFSuite) BeforeTest(suiteName, testName string) {
	s.eventCalled = false
	...
	s.sut = NewCRCSF(localEntity, s.Event)
	_ = s.sut.AddFeatures()
	s.sut.AddUseCase()
		...
		EntityInformation: []model.NodeManagementDetailedDiscoveryEntityInformationType{
			{
				Description: &model.NetworkManagementEntityDescriptionDataType{
					...
					EntityType: util.Ptr(model.EntityTypeTypeHvacRoom),
				},
			},
		},
