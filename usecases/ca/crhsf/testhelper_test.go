...
func TestCaCRHSFSuite(t *testing.T) {
	suite.Run(t, new(CaCRHSFSuite))
}

type CaCRHSFSuite struct {
	suite.Suite
...
func (s *CaCRHSFSuite) Event(ski string, device spineapi.DeviceRemoteInterface, entity spineapi.EntityRemoteInterface, event api.EventType) {
	s.eventCalled = true
}

func (s *CaCRHSFSuite) BeforeTest(suiteName, testName string) {
	s.eventCalled = false
	...
	s.sut = NewCRHSF(localEntity, s.Event)
	_ = s.sut.AddFeatures()
	s.sut.AddUseCase()
