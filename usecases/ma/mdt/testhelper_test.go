...
func TestMaMDTSuite(t *testing.T) {
	suite.Run(t, new(MaMDTSuite))
}

type MaMDTSuite struct {
	suite.Suite
...
func (s *MaMDTSuite) Event(ski string, device spineapi.DeviceRemoteInterface, entity spineapi.EntityRemoteInterface, event api.EventType) {
	s.eventCalled = true
}

func (s *MaMDTSuite) BeforeTest(suiteName, testName string) {
	s.eventCalled = false
	...
	s.sut = NewMDT(localEntity, s.Event)
	s.sut.AddFeatures()
	s.sut.AddUseCase()
