...
func TestMaMRTSuite(t *testing.T) {
	suite.Run(t, new(MaMRTSuite))
}

type MaMRTSuite struct {
	suite.Suite
...
func (s *MaMRTSuite) Event(ski string, device spineapi.DeviceRemoteInterface, entity spineapi.EntityRemoteInterface, event api.EventType) {
	s.eventCalled = true
}

func (s *MaMRTSuite) BeforeTest(suiteName, testName string) {
	s.eventCalled = false
	...
	s.sut = NewMRT(localEntity, s.Event)
	s.sut.AddFeatures()
	s.sut.AddUseCase()
