...
func TestMaMRHSFSuite(t *testing.T) {
	suite.Run(t, new(MaMRHSFSuite))
}

type MaMRHSFSuite struct {
	suite.Suite
...
func (s *MaMRHSFSuite) Event(ski string, device spineapi.DeviceRemoteInterface, entity spineapi.EntityRemoteInterface, event api.EventType) {
	s.eventCalled = true
}

func (s *MaMRHSFSuite) BeforeTest(suiteName, testName string) {
	s.eventCalled = false
	...
	s.sut = NewMRHSF(localEntity, s.Event)
	_ = s.sut.AddFeatures()
	s.sut.AddUseCase()
