...
func TestMaMDSFSuite(t *testing.T) {
	suite.Run(t, new(MaMDSFSuite))
}

type MaMDSFSuite struct {
	suite.Suite
...
func (s *MaMDSFSuite) Event(ski string, device spineapi.DeviceRemoteInterface, entity spineapi.EntityRemoteInterface, event api.EventType) {
	s.eventCalled = true
}

func (s *MaMDSFSuite) BeforeTest(suiteName, testName string) {
	s.eventCalled = false
	...
	s.sut = NewMDSF(localEntity, s.Event)
	_ = s.sut.AddFeatures()
	s.sut.AddUseCase()

	s.remoteDevice, s.dhwCircuitEntity = setupDevices(s.service, s.T())
}
