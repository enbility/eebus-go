...
func TestMaMRCSFSuite(t *testing.T) {
	suite.Run(t, new(MaMRCSFSuite))
}

type MaMRCSFSuite struct {
	suite.Suite
...
func (s *MaMRCSFSuite) Event(ski string, device spineapi.DeviceRemoteInterface, entity spineapi.EntityRemoteInterface, event api.EventType) {
	s.eventCalled = true
}

func (s *MaMRCSFSuite) BeforeTest(suiteName, testName string) {
	s.eventCalled = false
	...
	s.sut = NewMRCSF(localEntity, s.Event)
	_ = s.sut.AddFeatures()
	s.sut.AddUseCase()
